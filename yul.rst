...
1. Yul 的程序具有可读性，即便它是由编译器从 Solidity 或别的高级语言编译生成。
2. 控制流应该易于理解，并且有助于手工检查，形式化验证及优化。
3. Yul 到字节码的转化应该尽可能的简单。
4. Yul 应该适合做程序的整体优化。

为了达成前两个目标，Yul 提供一些高阶结构，如 ``for`` 循环语句, ``if`` 语句, ``switch`` 语句以及函数调用。
这些应该足够表示汇编程序的控制流了。
因此，不再为 ``SWAP``, ``DUP``, ``JUMP``, ``JUMPI`` 提供明确的语句，因为前两个混淆了数据流，后两个使控制流变得混乱。
此外，``mul(add(x, y), 7)`` 形式的函数语句比 ``7 y x add mul`` 这样的纯操作码语句更易读，前者更容易看出操作数和操作码的对应关系。

尽管 Yul 是为堆栈式机器设计的，但它并不会显出堆栈本身的复杂性。
程序员或审计人员不必再为堆栈而费脑。

第三个目标是能够用很规则的方法将高阶结构编译为字节码。
汇编器唯一的非原生操作是用户定义的标识符的查找和栈上局部变量的清除。

为了避免像值和引用这样的概念之间的混淆，Yul 设计为了静态类型。
同时，默认类型（通常是目标机器的整型）可以省略以提高可读性。
...
也可以使用 for 循环来代替递归来实现同样的功能。这里，``lt(a, b)`` 计算 ``a`` 是否小于 ``b``。
...
这将使用 `Yul 对象 <yul-object>`_ 以便将代码当作数据去部署合约。
Yul 独立形式可用于命令行编译器（使用 ``--strict-assembly``）以及 :ref:`json接口标准 <compiler-api>`。
...
Yul“对象”由代码、数据和子对象组成。详情请见下面的 `Yul 对象 <yul-object>`_。
在本节中，我们只关心对象的代码部分。
代码部分总是由用大括号分隔的块组成。
大多数工具只支持对象包含一个代码块。
